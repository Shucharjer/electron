...
//  2025-01-06: Vulkan: Added more ImGui_ImplVulkanH_XXXX helper functions to
//  simplify our examples. 2024-12-11: Vulkan: Fixed setting
...
//  requires '#define ImTextureID ImU64'.
//  (#914). 2021-10-15: Vulkan: Call vkCmdSetScissor() at the end of render a
//  full-viewport to reduce likelihood of issues with people using
...
static void CreateOrResizeBuffer(
    VkBuffer& buffer, VkDeviceMemory& buffer_memory, VkDeviceSize& buffer_size,
...
void ImGui_ImplVulkan_RenderDrawData(
    ...
    if (wrb->FrameRenderBuffers.Size == 0) {
        ...
        memset(
            (void*)wrb->FrameRenderBuffers.Data, 0,
    ...
    if (draw_data->TotalVtxCount > 0) {
        ...
        if (rb->IndexBuffer == VK_NULL_HANDLE ||
            ...
            CreateOrResizeBuffer(
                rb->IndexBuffer, rb->IndexBufferMemory, rb->IndexBufferSize,
        ...
        for (int n = 0; n < draw_data->CmdListsCount; n++) {
            ...
            memcpy(
                vtx_dst, draw_list->VtxBuffer.Data,
                draw_list->VtxBuffer.Size * sizeof(ImDrawVert));
            memcpy(
                idx_dst, draw_list->IdxBuffer.Data,
        ...
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++) {
            ...
            if (pcmd->UserCallback != nullptr) {
                ...
                if (pcmd->UserCallback == ImDrawCallback_ResetRenderState)
                    ImGui_ImplVulkan_SetupRenderState(
                        draw_data, pipeline, command_buffer, rb, fb_width,
                        fb_height);
                else
...
bool ImGui_ImplVulkan_CreateFontsTexture() {
    ...
    if (bd->TexCommandPool == VK_NULL_HANDLE) {
        ...
        vkCreateCommandPool(
            v->Device, &info, v->Allocator, &bd->TexCommandPool);
    }
    ...
    ImGui_ImplVulkan_Texture* backend_tex = &bd->FontTexture;
    {
        ...
        err =
            vkCreateImage(v->Device, &info, v->Allocator, &backend_tex->Image);
        check_vk_result(err);
        VkMemoryRequirements req;
        ...
        err = vkAllocateMemory(
            v->Device, &alloc_info, v->Allocator, &backend_tex->Memory);
        check_vk_result(err);
        err = vkBindImageMemory(
            v->Device, backend_tex->Image, backend_tex->Memory, 0);
        check_vk_result(err);
    }
    ...
    // Create the Image View:
    {
        ...
        err                              = vkCreateImageView(
            v->Device, &info, v->Allocator, &backend_tex->ImageView);
        check_vk_result(err);
    }

    // Create the Descriptor Set:
    backend_tex->DescriptorSet = ImGui_ImplVulkan_AddTexture(
        bd->TexSampler, backend_tex->ImageView,
    ...
    VkBuffer upload_buffer;
    {
        ...
        buffer_info.sharingMode        = VK_SHARING_MODE_EXCLUSIVE;
        err                            = vkCreateBuffer(
            v->Device, &buffer_info, v->Allocator, &upload_buffer);
        check_vk_result(err);
        VkMemoryRequirements req;
        ...
        err = vkBindBufferMemory(
            v->Device, upload_buffer, upload_buffer_memory, 0);
        check_vk_result(err);
    }
    ...
    // Copy to Image:
    {
        ...
        vkCmdPipelineBarrier(
            bd->TexCommandBuffer, VK_PIPELINE_STAGE_HOST_BIT,
        ...
        vkCmdPipelineBarrier(
            bd->TexCommandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT,
...
bool ImGui_ImplVulkan_CreateDeviceObjects() {
    ...
    if (v->DescriptorPoolSize != 0) {
        ...
        err = vkCreateDescriptorPool(
            v->Device, &pool_info, v->Allocator, &bd->DescriptorPool);
        check_vk_result(err);
    }
    ...
    ImGui_ImplVulkan_CreatePipeline(
        v->Device, v->Allocator, v->PipelineCache, v->RenderPass,
...
void ImGui_ImplVulkan_DestroyDeviceObjects() {
    ...
    if (bd->TexCommandBuffer) {
        vkFreeCommandBuffers(
            v->Device, bd->TexCommandPool, 1, &bd->TexCommandBuffer);
...
static void ImGui_ImplVulkan_LoadDynamicRenderingFunctions(
    ...
    ImGuiImplVulkanFuncs_vkCmdBeginRenderingKHR =
        reinterpret_cast<PFN_vkCmdBeginRenderingKHR>(loader_func(
            api_version < VK_API_VERSION_1_3 ? "vkCmdBeginRenderingKHR"
                                             : "vkCmdBeginRendering",
            user_data));
    ImGuiImplVulkanFuncs_vkCmdEndRenderingKHR =
        reinterpret_cast<PFN_vkCmdEndRenderingKHR>(loader_func(
            api_version < VK_API_VERSION_1_3 ? "vkCmdEndRenderingKHR"
                                             : "vkCmdEndRendering",
            user_data));
}
    #endif
...
bool ImGui_ImplVulkan_LoadFunctions(
    ...
    ImGui_ImplVulkan_LoadDynamicRenderingFunctions(
        api_version, loader_func, user_data);
        #endif
    #else
...
bool ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info) {
        ...
        ImGui_ImplVulkan_LoadDynamicRenderingFunctions(
            ...
            [](const char* function_name, void* user_data) {
                return vkGetInstanceProcAddr(
                    (VkInstance)user_data, function_name);
            },
            (void*)info->Instance);
        #endif
    ...
    #else
        IM_ASSERT(
            0 && "Can't use dynamic rendering when neither VK_VERSION_1_3 or "
    ...
    if (info->DescriptorPool !=
        VK_NULL_HANDLE) // Either DescriptorPool or DescriptorPoolSize must
                        // be set, not both!
...
uint32_t
    ...
    vkGetPhysicalDeviceQueueFamilyProperties(
        physical_device, &count, queues_properties.Data);
...
void ImGui_ImplVulkanH_CreateWindowSwapChain(
    ...
    // Create Swapchain
    {
        ...
        if (info.minImageCount < cap.minImageCount)
            ...
            cap.maxImageCount != 0 && info.minImageCount > cap.maxImageCount)
        ...
        memset(
            wd->FrameSemaphores.Data, 0, wd->FrameSemaphores.size_in_bytes());
        for (uint32_t i = 0; i < wd->ImageCount; i++)
