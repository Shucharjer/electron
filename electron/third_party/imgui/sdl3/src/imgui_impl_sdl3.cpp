...
//  2025-03-10: When dealing with OEM keys, use scancodes instead of translated
//  keycodes to choose ImGuiKey values. (#7136, #7201, #7206, #7306, #7670,
//  #7672, #8468) 2025-02-26: Only start SDL_CaptureMouse() when mouse is being
//  dragged, to mitigate issues with e.g.Linux debuggers not claiming capture
//  back. (#6410, #3650) 2025-02-24: Avoid calling SDL_GetGlobalMouseState()
//  when mouse is in relative mode. 2025-02-18: Added ImGuiMouseCursor_Wait and
...
//  ignores events intended for other SDL windows.
//  (#7853) 2024-07-22: Update for SDL3 api changes: SDL_GetGamepads() memory
//  ownership change.
//  (#7807) 2024-07-18: Update for SDL3 api changes: SDL_GetClipboardText()
//  memory ownership change.
//  (#7801) 2024-07-15: Update for SDL3 api changes: SDL_GetProperty() change to
//  SDL_GetPointerProperty(). (#7794) 2024-07-02: Update for SDL3 api changes:
//  SDLK_x renames and SDLK_KP_x removals (#7761, #7762). 2024-07-01: Update for
//  SDL3 api changes: SDL_SetTextInputRect() changed to SDL_SetTextInputArea().
//  2024-06-26: Update for SDL3 api changes:
...
#include "imgui.h"
    ...
    // Clang warnings with -Weverything
        ...
        #pragma clang diagnostic ignored                                       \
            "-Wimplicit-int-float-conversion" // warning: implicit conversion
                                              // from 'xxx' to 'float' may lose
                                              // precision
    #endif
...
bool ImGui_ImplSDL3_ProcessEvent(const SDL_Event* event) {
    ImGui_ImplSDL3_Data* bd = ImGui_ImplSDL3_GetBackendData();
    IM_ASSERT(
        bd != nullptr && "Context or backend not initialized! Did you call "
                         "ImGui_ImplSDL3_Init()?");
    ImGuiIO& io = ImGui::GetIO();
...
static void ImGui_ImplSDL3_UpdateMouseData() {
    ...
    // be very inconvenient, so to migitate the issue we wait until mouse has
    ...
    if (is_app_focused) {
        ...
        if (bd->MouseCanUseGlobalState && bd->MouseButtonsDown == 0 &&
            ...
            io.AddMousePosEvent(
                mouse_x_global - window_x, mouse_y_global - window_y);
        }
    }
}
...
void ImGui_ImplSDL3_SetGamepadMode(
    ImGui_ImplSDL3_GamepadMode mode, SDL_Gamepad** manual_gamepads_array,
...
static void ImGui_ImplSDL3_UpdateGamepadButton(
    ImGui_ImplSDL3_Data* bd, ImGuiIO& io, ImGuiKey key,
...
static void ImGui_ImplSDL3_UpdateGamepadAnalog(
    ImGui_ImplSDL3_Data* bd, ImGuiIO& io, ImGuiKey key, SDL_GamepadAxis axis_no,
    float v0, float v1) {
    float merged_value = 0.0f;
    for (SDL_Gamepad* gamepad : bd->Gamepads) {
        float vn = Saturate(
            (float)(SDL_GetGamepadAxis(gamepad, axis_no) - v0) /
            (float)(v1 - v0));
        if (merged_value < vn)
            merged_value = vn;
    }
...
static void ImGui_ImplSDL3_UpdateGamepads() {
    ...
    ImGui_ImplSDL3_UpdateGamepadAnalog(
        bd, io, ImGuiKey_GamepadR2, SDL_GAMEPAD_AXIS_RIGHT_TRIGGER, 0.0f,
        32767);
    ImGui_ImplSDL3_UpdateGamepadButton(
    ...
    ImGui_ImplSDL3_UpdateGamepadAnalog(
        bd, io, ImGuiKey_GamepadLStickLeft, SDL_GAMEPAD_AXIS_LEFTX,
        -thumb_dead_zone, -32768);
    ImGui_ImplSDL3_UpdateGamepadAnalog(
        bd, io, ImGuiKey_GamepadLStickRight, SDL_GAMEPAD_AXIS_LEFTX,
        +thumb_dead_zone, +32767);
    ImGui_ImplSDL3_UpdateGamepadAnalog(
        bd, io, ImGuiKey_GamepadLStickUp, SDL_GAMEPAD_AXIS_LEFTY,
        -thumb_dead_zone, -32768);
    ImGui_ImplSDL3_UpdateGamepadAnalog(
        bd, io, ImGuiKey_GamepadLStickDown, SDL_GAMEPAD_AXIS_LEFTY,
        +thumb_dead_zone, +32767);
    ImGui_ImplSDL3_UpdateGamepadAnalog(
        bd, io, ImGuiKey_GamepadRStickLeft, SDL_GAMEPAD_AXIS_RIGHTX,
        -thumb_dead_zone, -32768);
    ImGui_ImplSDL3_UpdateGamepadAnalog(
        bd, io, ImGuiKey_GamepadRStickRight, SDL_GAMEPAD_AXIS_RIGHTX,
        +thumb_dead_zone, +32767);
    ImGui_ImplSDL3_UpdateGamepadAnalog(
        bd, io, ImGuiKey_GamepadRStickUp, SDL_GAMEPAD_AXIS_RIGHTY,
        -thumb_dead_zone, -32768);
    ImGui_ImplSDL3_UpdateGamepadAnalog(
        bd, io, ImGuiKey_GamepadRStickDown, SDL_GAMEPAD_AXIS_RIGHTY,
        +thumb_dead_zone, +32767);
}
